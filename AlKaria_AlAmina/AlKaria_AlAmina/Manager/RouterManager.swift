    ...
    private func viewControllerForNavigation(screen: INaviagtion, modalePresentatioinStyle: UIModalPresentationStyle?) -> UIViewController {
        ...
        if let screen = screen as? AppScreen {
            ...
            case .ColorDifferentGame:
                guard let vc = ColorDifferentGameVc.instantiate(storyboardName: "Main") else {return UIViewController()}
                viewController = vc
                break
            case .ScoreScreen(score: let score, bestScore: let bestScore):
